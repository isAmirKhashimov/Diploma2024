...
namespace Diploma2
{
	internal class LinearSystem(double[,] a, double[] d)
	{
        private double[,] A = a;
        private double[] D = d;

		public void RecalcMAsConstant(int m, double value)
        {
            var n = D.Length;
            if (m >= n) throw new ArgumentException();

            var nA = new double[n - 1, n - 1];
            var nd = new double[n - 1];

            for (int i = 0; i < m; i++)
            {
                nd[i] = D[i] - A[i, m] * value;
